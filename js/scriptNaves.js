...
async function carregarPersonagens() {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        if (localStorage.getItem(nomeChave)) {
            ...
            listaDeSA = JSON.parse(localStorage.getItem(nomeChave));
        }
        else {
            ...
            if (resposta.ok) {
                ...
                listaDeSA = json.results; // Pega só a lista de planetas

                // Salva agora para usar na próxima vez
                localStorage.setItem(nomeChave, JSON.stringify(listaDeSA));
            }
        }

        // 3. EXIBE (Essa parte roda independente de onde veio os dados)
        if (listaDeSA) {
            for (let b = 0; b < listaDeSA.length; b++) {
                exibierPeronagem(listaDeSA[b], c);
                c++;
            }
        }
    }
